...
export async function POST(request: NextRequest) {
  try {
    ...
    const immediateResponse = NextResponse.json({
      success: true,
      emailId: "queued_for_delivery",
      attachmentCount: fileIds ? fileIds.length : 0,
      message: "Email queued for delivery in background",
    });

    // Execute massive tasks (Google Drive Download -> NodeMailer -> MongoDB) in the background
    (async () => {
      try {
        ...
        console.error("[Email API] Fatal background error:", bgError);
      }
    })();

    // Instantly return the response to the user's browser
