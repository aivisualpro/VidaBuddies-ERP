...
export default function AppAdminSupplierLayout({ children, params }: { children: React.ReactNode, params: Promise<{ id: string }> }) {
  ...
  const { id } = use(params);
  const activeTab = pathname.includes("/supply-survey") ? "supply-survey" : pathname.includes("/survey") ? "survey" : pathname.includes("/documents") ? "documents" : pathname.includes("/history") ? "history" : "dashboard";
  return (
